...
var VIEWER = function () {
    ...
    function safeDistance() {
        ...
        // Step through all the objects in the scene and find the outer bounds of their volumes
        for (var i = 0; i < currentObjects.length; i++) {
            if (currentObjects[i].geometry.boundingBox === undefined || currentObjects[i].geometry.boundingBox === null) {
                // console.log("bBox Undefined");
                continue;
            }

            // Mins
            if (currentObjects[i].geometry.boundingBox.min.x < minX) minX = currentObjects[i].geometry.boundingBox.min.x;
            if (currentObjects[i].geometry.boundingBox.min.y < minY) minY = currentObjects[i].geometry.boundingBox.min.y;
            if (currentObjects[i].geometry.boundingBox.min.z < minZ) minZ = currentObjects[i].geometry.boundingBox.min.z;

            // Maxes
            if (currentObjects[i].geometry.boundingBox.max.x > maxX) maxX = currentObjects[i].geometry.boundingBox.max.x;
            if (currentObjects[i].geometry.boundingBox.max.y > maxY) maxY = currentObjects[i].geometry.boundingBox.max.y;
            if (currentObjects[i].geometry.boundingBox.max.z > maxZ) maxZ = currentObjects[i].geometry.boundingBox.max.z;
        }

        var centerX = 0.5 * (maxX - minX);
        var centerY = 0.5 * (maxY - minY);
        var centerZ = 0.5 * (maxZ - minZ);
        var centroid = new THREE.Vector3(centerX, centerY, centerZ);

        // Calculate the safe distance that we should be away from the object to get a good view
        var sphereSize = centroid.length() * 2.0;
